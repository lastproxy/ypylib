...
from ypylib.stat import bin_xyz
...
class Level2Files:
    ...
    def __init__(self, collection=6, nrt=False, **kw):
        ...
        self.c5nrt = r'nrt2.modaps.eosdis.nasa.gov/allData/1'
...
#===============================================================================
    def getUrl(self):
        ...
        dates.
        '''
        if (self.nrt is True): self.getCredential()
...
#===============================================================================
    def download(self, source=None, destination=None, skip_download=False):
        ...
        if self.nrt is True: otag = 'MODIS_NRT_C' + str(self.collection).strip()
        else: otag = 'MODIS_SCI_C' + str(self.collection).strip()

        self.local = os.path.join(os.environ['DATADIR']
                                  if destination is None else destination,
                                  otag, self.product, self.dt2yjd())

        if skip_download is True: return
        #------------------------------------------------------------ Print info
...
#===============================================================================
    def consolidateDailyAod(self, filepath=None, valid_time=['0000', '2400'],
        ...
        if (len(hdfiles) > 0):
            ...
            if self.collection == 6:
                ...
                if len(self.aodfields) < 6 :
                    self.aodfields[3:5] = ['Optical_Depth_Land_And_Ocean',
                                   'Deep_Blue_Aerosol_Optical_Depth_550_Land',
                                   'Quality_Assurance_Land']

                for hdfi in hdfiles:
                    lon, lat, time, aodc, aodd, qfl = \
                    get_sd(FILENAME=hdfi, SDSNAME=self.aodfields, QUIET=True)
        ...
        z[2] = 'daily'
        if self.nrt is not True: z[4] = 'SCI'
...
#===============================================================================
    def writecsvAod(self, hdfile, csvfile=None, deletehdf=False):
        ...
        if (vaod.size > 0):
            if (deletehdf is True): os.remove(hdfile)
...
#===============================================================================
    def writencDailyAod(self, ncfile=None, filepath=None,
        ...
        times.long_name = 'TAI Time at Start of Scan replicated across the swath'
        times.units = 'Seconds since 1993-1-1 00:00:00.0 0'
        lons = nc.createVariable('lon', 'f', 't' , zlib=True,
                                 least_significant_digit=3)
        lons.long_name = 'Geodetic Longitude'
        lons.standard_name = 'longitude'
        lons.units = 'degrees_east'
        lats = nc.createVariable('lat', 'f', 't', zlib=True,
                                 least_significant_digit=3)
        lats.long_name = 'Geodetic Latitude'
        lats.standard_name = 'latitude'
        lats.units = 'degrees_north'
        aods = nc.createVariable('aod550', 'f', 't' , zlib=True,
        ...
        qfs = nc.createVariable('quality_flag', 'h', 't', zlib=True,
                                least_significant_digit=1)
        qfs.long_name = 'Combined Dark Target, Deep Blue Aerosol ' + \
                        'Confidence Flag'
        qfs.flag_values = [0, 1, 2, 3]
        qfs.flag_meanings = ['No Confidence (or fill), Marginal, ' +
                             'Good, Very Good']
        qfs.units = '1'
        qfs.valid_range = [0, 3]
...
#===============================================================================
    def plotDailyAod(self, rebin=(0.5, 0.5), figsize=(8, 5),
        ...
        # Consolidate files
        _time, lon, lat, aod, _qf = self.consolidateDailyAod(filepath=filepath,
        ...
        # Add extra grid lines
#         ax.xaxis.set_major_locator(plt.MultipleLocator(30.0))
#         ax.xaxis.set_minor_locator(plt.MultipleLocator(10.0))
#         ax.yaxis.set_major_locator(plt.MultipleLocator(30.0))
#         ax.yaxis.set_minor_locator(plt.MultipleLocator(10.0))
#         ax.grid(which='major', axis='x', linewidth=0.1, linestyle='-', color='0.75')
#         ax.grid(which='minor', axis='x', linewidth=0.1, linestyle='-', color='0.85')
#         ax.grid(which='major', axis='y', linewidth=0.1, linestyle='-', color='0.75')
#         ax.grid(which='minor', axis='y', linewidth=0.1, linestyle='-', color='0.85')
#         ax.set_xticklabels([])
#         ax.set_yticklabels([])
